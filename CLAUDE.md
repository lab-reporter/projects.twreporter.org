...
## 🌳 Git 工作流程規範
- **生產分支**: `main` - 穩定版本，合併來自開發分支的穩定功能
- **3D 開發分支**: `dev-3d` - React Three Fiber 3D 解決方案開發
- **2D 開發分支**: `dev-2d` - 傳統 2D 解決方案開發 (跳脫 R3F)
- **GitHub Repository**: https://github.com/itisalongway574/r3f-10th-repo
- **開發模式**: 雙軌開發架構，根據技術路線選擇對應分支
- **分支策略**: 
  - `dev-3d`: 繼續優化現有 3D/R3F 實作
  - `dev-2d`: 探索純 2D CSS/JS 動畫解決方案
  - 兩個分支可獨立發展，最終評估效果後選擇最佳方案合併到 `main`
...
## 🔀 分支架構重構 (2025-06-29)
- **技術路線分歧**: 建立雙軌開發架構，同時探索 3D 和 2D 解決方案
- **分支重新命名**: `dev` → `dev-3d`，強調 3D/R3F 技術路線
- **新增 2D 分支**: `dev-2d` 探索傳統 CSS/JS 動畫實作方式
- **開發策略**: 
  - `dev-3d`: 基於現有 R3F 架構，持續優化效能與體驗
  - `dev-2d`: 從零開始設計 2D 實作，避免 3D 渲染的效能負擔
  - 平行開發，最終比較兩種方案的效果、效能和維護性
- **決策背景**: 3D 方案雖然視覺效果佳，但在低階設備上可能有效能問題，2D 方案可提供更好的兼容性和載入速度

---
*最後更新: 2025-06-29 - 分支架構重構完成，建立 dev-3d/dev-2d 雙軌開發模式*