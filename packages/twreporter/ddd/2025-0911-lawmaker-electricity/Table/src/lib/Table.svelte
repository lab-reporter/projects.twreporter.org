<script>
    ...
    function identityInfoFor(cellText) {
        ...
        for (const part of parts) {
            const match =
                $membersIdentityByName && $membersIdentityByName.get
    ...
    function getCellParts(cellText) {
        ...
        const parts = lines.map((name, idx) => {
            ...
            const color =
                party && PARTY_COLOR_MAP[party] ? PARTY_COLOR_MAP[party] : "";
        ...
        {#each tableData as row}
            <tr>
                {#each columns as col, i}
                    {#if isSpannedByPrev(row, i)}{:else}
                        <td
                            colspan={getColspan(row, i)}
                            on:mouseenter={(e) =>
                                onEnter(e, identityInfoFor(row[col]))}
                            on:mousemove={onMove}
...
{#if tooltip}
    <Tooltip x={tooltip.x} y={tooltip.y} identities={tooltip.identities} />
{/if}
